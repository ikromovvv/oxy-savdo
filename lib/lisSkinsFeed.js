...
// LIS-SKINS to'liq eksporti juda katta (yuz(lab) MB). Uni serverless
// funksiyada oqim orqali tahlil qilish Vercel'da xotira/vaqt limitiga uriladi
// (OOM, 10s timeout). Shuning uchun skinlar katalogi standart holda O'CHIQ.
// Yoqish uchun .env.local ga: SKINS_ENABLED=true
// Yoqilganda ham parse MAX_CATALOG_ITEMS ta elementdan keyin to'xtatiladi.
const SKINS_ENABLED = process.env.SKINS_ENABLED === 'true';
const MAX_CATALOG_ITEMS = Number(process.env.SKINS_MAX_ITEMS) || 4000;
...
async function buildCatalog() {
  ...
  await streamFeedItems((it) => {
    if (items.length >= MAX_CATALOG_ITEMS) return false; // yetarli — oqimni to'xtatamiz
    if (!it.name) return;
...
async function getCatalog() {
  // Skinlar o'chirilgan bo'lsa — feedga umuman tegmaymiz (Vercel'da OOM sabab)
  if (!SKINS_ENABLED) {
    return { items: [], weaponTypes: [], ts: Date.now(), disabled: true };
  }
