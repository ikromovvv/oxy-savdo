...
export async function listOrders({ status, statuses, steamid } = {}) {
  if (dbEnabled) {
    await ensureMigrations();
    const sql = db();
    const conds = [];
    if (Array.isArray(statuses) && statuses.length) conds.push(sql`status in ${sql(statuses)}`);
    else if (status && status !== 'all') conds.push(sql`status = ${status}`);
    if (steamid) conds.push(sql`steam->>'steamid' = ${String(steamid)}`);

    const where = conds.length
      ? sql`where ${conds.reduce((a, b) => sql`${a} and ${b}`)}`
      : sql``;
    const rows = await sql`select * from oxy.orders ${where} order by created_at desc`;
    return rows.map(rowToOrder);
  }

  const all = await readAll();
  let list = all;
  if (Array.isArray(statuses) && statuses.length) list = list.filter((o) => statuses.includes(o.status));
  else if (status && status !== 'all') list = list.filter((o) => o.status === status);
  if (steamid) list = list.filter((o) => o.steam?.steamid === String(steamid));
  return list;
}
